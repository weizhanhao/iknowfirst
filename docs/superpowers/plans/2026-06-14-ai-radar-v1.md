...
## Phase 9 — YouTube 热度采样接入调度（v1 收尾）

### Task 19: YouTube 热度采样 + 飙升补推

**设计决定（用户确认）：增速飙升 → 补推一条「🔥 热度飙升」重磅卡片**（带去重标记，不重复推）。窗口用 `created_at`（naive，一致；发现时间≈发布时间）。仅采样命中过关键词的 YouTube 条目（status 不在 seen/skipped），省配额。`fetch` 函数注入 sampler 以便测试免联网。完整代码与逐步 TDD 见实现者收到的任务规格；要点：

- `engagement/youtube_api.py`：`extract_video_id(item)`（`yt:video:ID` / `?v=` / `youtu.be/`）+ `fetch_stats(video_id, api_key) -> (views, likes, comments)`（YouTube Data API v3，likeCount/commentCount 缺省 0，无 item 抛错）。
- `Item` 加 `engagement_promoted: bool=False` 列。
- Repository 加：`youtube_tracked_items(created_after)`（youtube + created_at≥cutoff + status∉{seen,skipped}）、`add_engagement_sample(..., sampled_at=None)`、`likes_samples_for(item_id)`、`mark_engagement_promoted(item_id)`。
- `Notifier.push_spike(title, url, author, source_type, likes_per_hour)`：组「🔥 热度飙升」卡片即时推。
- `EngagementSampler.run_once(now)`：对窗口内 youtube 条目拉 stats→存样本→`likes_per_hour`→增速≥阈值且未 promoted 则 `push_spike`+标记。无 video_id 跳过、fetch 异常吞掉。
- `main.py`：`YOUTUBE_API_KEY` 存在且 `engagement.enabled` 时,注册每 `sample_interval_minutes` 的 `sampler.run_once(datetime.now())`。

验收:`pytest -q` 全绿(新增约 11 测试)+ `import iknowfirst.main` 无错。Commit:`feat: youtube engagement sampling job with spike promotion`。
...
### 终审记录的 v1 已接受取舍(留 v2,非 bug)

最终整体 review 已修复 1 Critical(配置环境变量插值)+ 1 High(RSS summary→raw_text)。以下 Important 项**有意留到 v2**,在此显式记录:

- **`analyses` / `pushes` 表暂不持久化**:v1 的 `Pipeline` 只更新 `Item.status`(analyzed/error),不写 `analyses`(含 `model_used`)/`pushes` 记录。代价:无推送审计、无跨重启的推送去重(进程重启后已采集未处理的 `new` 条目不会被重新处理——v1 容忍)。v2 接入 `AnalysisRepository`/`PushRepository`。
- **汇总队列为内存态**:`Notifier._digest` 在进程内,崩溃/重启会丢失当批未推的普通条目。v2 用 `pushes` 表做持久化待推队列。
- **LLM fallback 暂未接线**:`build_llm` 的 `fallback=None`;`cfg.llm.fallback`(示例里的 `deepseek`)v1 未消费(配置缺备用端点的 base_url/model/key)。v2 扩展配置后接入真实备用实例。
- **`trendscout.lookback_days` 暂未做日期过滤**:v1 用 `items_by_status("skipped")`(上限 500 条)近似;v2 按发布时间窗口过滤。
